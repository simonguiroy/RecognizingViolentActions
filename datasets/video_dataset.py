...
# For now, this dataset class should only be used to evaluate a model, and not for training
...
class VideoDataset(Dataset):
    ...
    def __init__(self,
        ...
        Args:
            ...
            stream (string): Type of stream for model input [rgb/flow/joint]
    ...
    def __getitem__(self, idx):
        ...
        if self.stream == 'rgb' or self.stream == 'joint':
            sample['videos']['video_rgb'] = torch.from_numpy(preprocess_rgb(self.root_dir + video_filename))
        if self.stream == 'flow' or self.stream == 'joint':
            sample['videos']['video_flow'] = preprocess_flow(self.root_dir + video_filename)
